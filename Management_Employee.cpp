...
// Structure of Employee details for computation
struct employee {
	string name;
	long int code;
	string designation;
...
// Array of Employees to store data in the form of the Structure
employee emp[max], tempemp[max],sortemp[max], sortemp1[max];

// Function to build based on the given datatype
void build()
{
	...
	if (num > 20) {
		cout << "Maximum number of entries are 10\n";
		num = 20;
	}
...
void showMenu()
{
	...
	// Input for picking option
	cin >> option;

	// Call function on basis of option picked
	if (option == 1) {
		build();
	}
	else if (option == 2) {
